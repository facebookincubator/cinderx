...
PyObject* Ci_cfunction_vectorcall_typed_0(
    PyObject* func,
    PyObject* const* args,
    size_t nargsf,
    PyObject* kwnames) {
  ...
  Ci_PyTypedMethodDef* def =
      (Ci_PyTypedMethodDef*)Cix_cfunction_enter_call(tstate, func);
  if (def == NULL) {
    return NULL;
  }
...
#define CONV_ARGS(n)                                                     \
  ...
  for (Py_ssize_t i = 0; i < n; i++) {                                   \
    final_args[i] = _PyClassLoader_ConvertArg(                           \
        self, def->tmd_sig[i], i, nargsf, args, &error);                 \
    if (error) {                                                         \
      if (!PyErr_Occurred()) {                                           \
        PyObject* funcstr = _PyObject_FunctionStr(func);                 \
        if (funcstr != NULL) {                                           \
...
PyObject* Ci_cfunction_vectorcall_typed_1(
    PyObject* func,
    PyObject* const* args,
    size_t nargsf,
    PyObject* kwnames) {
...
PyObject* Ci_cfunction_vectorcall_typed_2(
    PyObject* func,
    PyObject* const* args,
    size_t nargsf,
    PyObject* kwnames) {
...
vectorcallfunc Ci_PyCMethod_New_METH_TYPED(PyMethodDef* method) {
  ...
  switch (arg_cnt) {
    case 0:
      vectorcall = Ci_cfunction_vectorcall_typed_0;
      break;
    case 1:
      vectorcall = Ci_cfunction_vectorcall_typed_1;
      break;
    case 2:
      vectorcall = Ci_cfunction_vectorcall_typed_2;
      break;
    default:
      PyErr_Format(
          PyExc_SystemError,
