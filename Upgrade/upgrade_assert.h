...
enum {
  ...
  // Missing interp->modules, import_func, modules_by_index
  MISSING_INTERP_MOD_FIELDS,
  INCOMPLETE_PY_AWAITER,
  AST_UPDATES,
  MISSING_CO_NOFREE,
  // Ci_Py_TPFLAGS_IS_STATICALLY_DEFINED, Ci_Py_VECTORCALL_INVOKED_METHOD, CO_STATICALLY_COMPILED
  NEED_STATIC_FLAGS,
  PYLONG_DATA_CHANGED,
  MISSING_PY_TYPEFLAGS_FROZEN,
  // This is a macro and used from C code so a bit tricky to stub.
  MISSING_PyHeapType_GET_MEMBERS,
  MISSING_CINDERX_INITIALIZED_HOOK,
  PROFILING_CHANGED,
  MISSING_FUNC_EVENT_MODIFY_QUALNAME,
