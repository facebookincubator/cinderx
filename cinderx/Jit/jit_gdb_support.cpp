...
struct ELFObjectContext {
  ...
  uint32_t stack_size{0}; /* Stack adjustment for trace itself. */
  size_t objsize{0}; /* Final size of ELF object. */
  ELFObject obj; /* In-memory ELF object. */
};

/* Template for in-memory ELF header. */
const ELFHeader elfhdr_template = {
    .emagic = {0x7f, 'E', 'L', 'F'},
    .eclass = 2,
    .eendian = 1,
    .eversion = 1,
...
void elfctx_build_object(ELFObjectContext* ctx);

int register_elf_ctx(ELFObjectContext* ctx, const char* type, void* ptr) {
  elfctx_build_object(ctx);

  // Now we allocate enough space for both the JITCodeEntry and the
  // ELFObject.
  size_t elf_object_size = ctx->objsize;
  char* raw =
  ...
  memcpy(elf_object_start, &ctx->obj, elf_object_size);
...
uint32_t elfctx_append_string(ELFObjectContext* ctx, const char* str) {
  uint8_t* p = ctx->p;
...
void elfctx_append_sleb128(ELFObjectContext* ctx, int32_t v) {
  uint8_t* p = ctx->p;
...
void elfctx_append_uleb128(ELFObjectContext* ctx, uint32_t v) {
  uint8_t* p = ctx->p;
  for (; v >= 0x80; v >>= 7) {
    *p++ = (char)((v & 0x7f) | 0x80);
  }
...
void elf_secthdr(ELFObjectContext* ctx) {
...
void elf_init_symtab(ELFObjectContext* ctx) {
  ELFSymbol* sym;
...
void elf_init_debuginfo(ELFObjectContext* ctx) {
  uint8_t* p = ctx->p;
...
void elf_init_debugabbrev(ELFObjectContext* ctx) {
  uint8_t* p = ctx->p;
...
void elf_init_debugline(ELFObjectContext* ctx) {
  uint8_t* p = ctx->p;
...
void elf_init_section(ELFObjectContext* ctx, int sect, ELFSectionInitFn init) {
  ctx->startp = ctx->p;
...
int register_pycode_debug_symbol(
  ...
  size_t code_size = compiled_func.code.size();
