...
def get_gdb_version():
    ...
    if major_version < 7:
        raise unittest.SkipTest(
            ...
            "embedding. Saw %s.%s:\n%s" % (major_version, minor_version, version)
        )
...
def run_gdb(*args, **env_vars):
    ...
    if (gdb_major_version, gdb_minor_version) >= (7, 4):
        ...
        checkout_hook_path = os.path.join(
            os.path.dirname(sys.executable), "python-gdb.py"
        )
...
id(foo.__code__)""",
            ...
            cmds_after_breakpoint=[
                "print (PyFrameObject*)(((PyCodeObject*)v)->co_mutable->co_zombieframe)"
            ],
        )
        self.assertTrue(
            re.match(
