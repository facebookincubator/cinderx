...
function run() {
    local mode=$1
    buck2 run @fbcode//mode/"${mode}" fbcode//cinderx:python3.10 -- -X jit-all -X jit-multithreaded-compile-test -X jit-batch-compile-workers="${WORKERS}" -m test_cinderx.multithreaded_compile_test
}
