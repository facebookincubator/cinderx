...
# Opening, intermediate, and closing directives of a conditional block. Used to
# detect and strip empty conditional blocks from the generated output.
CPP_CONDITIONAL_START_PATTERN: re.Pattern[str] = re.compile(r"^\s*#\s*if(def|ndef)?\b")
CPP_CONDITIONAL_MID_PATTERN: re.Pattern[str] = re.compile(
    r"^\s*#\s*el(if(def|ndef)?|se)\b"
)
CPP_CONDITIONAL_END_PATTERN: re.Pattern[str] = re.compile(r"^\s*#\s*endif\b")
...
@dataclass
class _OpenBlock:
    # Index in the output of this block's opening directive.
    start: int
    # Whether any real content has been seen inside the block yet.
    has_content: bool = False


def _close_ifdef_block(stack: list[_OpenBlock], result: list[str]) -> bool:
    """Handle an `#endif`: pop the innermost open block and, if it was empty,
    drop it from `result`. A non-empty block marks its parent as non-empty.
    Returns whether a block was removed."""
    if not stack:
        return False
    block = stack.pop()
    if not block.has_content:
        del result[block.start :]
        return True
    if stack:
...
def _strip_empty_ifdef_blocks_once(lines: list[str]) -> tuple[list[str], bool]:
    """Run a single removal pass. Returns the rewritten lines and whether any
    empty block was removed."""
    result: list[str] = []
    stack: list[_OpenBlock] = []
    removed = False
    for line in lines:
        if CPP_CONDITIONAL_START_PATTERN.match(line):
            stack.append(_OpenBlock(len(result)))
        elif CPP_CONDITIONAL_END_PATTERN.match(line):
            result.append(line)
            removed |= _close_ifdef_block(stack, result)
            continue
        elif not CPP_CONDITIONAL_MID_PATTERN.match(line) and line.strip() and stack:
            stack[-1].has_content = True
        result.append(line)
    return result, removed


def remove_empty_ifdef_blocks(lines: list[str]) -> list[str]:
    """Remove conditional blocks whose body is empty, i.e. blocks that contain
    only the conditional directives themselves (`#if`/`#ifdef`/`#elif`/`#else`/
    `#endif`) and blank lines. Removing an inner block can leave an enclosing
    block empty, so this iterates to a fixed point."""
    removed = True
    while removed:
        lines, removed = _strip_empty_ifdef_blocks_once(lines)
    return lines
    ...
    def _generate_output(self) -> None:
        ...
        for line in self.input_lines:
            match line:
                ...
                case str():
                    out.append(line)
        self.output_lines = remove_empty_ifdef_blocks(out)
